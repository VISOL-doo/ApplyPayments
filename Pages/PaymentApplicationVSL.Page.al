page 71357860 "PaymentApplication_VSL"
{
        ...
        area(content)
        {
            ...
            group("Open Entries")
            {
                ...
                repeater(Control28)
                {
                    ...
                    field("Ship to name_VSL"; "Ship to name_VSL")
                    {
                        ApplicationArea = Basic, Suite;
                        Editable = false;
                    ...
                    field("Payment reference_VSL"; "Payment reference_VSL")
                    {
                        ApplicationArea = Basic, Suite;
                        Editable = false;
        ...
        area(processing)
        {
            ...
            group(Show)
            {
                ...
                action(SortEntriesBasedOnProbability)
                {
                    ...
                    trigger OnAction()
                    var
                        GetBankStmtLineCandidates: Codeunit "GetBankStmt.LineCandidates_VSL";
                    begin
                        Rec.Reset();
                        Rec.DeleteAll();
    ...
    begin
        ...
        CODEUNIT.Run(CODEUNIT::"GetBankStmt.LineCandidates_VSL", Rec);
        Rec.SetCurrentKey("Sorting Order", "Stmt To Rem. Amount Difference");
        Rec.Ascending(true);
        if BankAccReconLine."Account No." <> '' then begin
            Rec.SetRange("Account No.", BankAccReconLine."Account No.");
    ...
    begin
        ...
        //Rec.OnSetBankAccReconcLine(BankAccReconLine);
    end;
    ...
    begin
        if (Rec."Match Confidence" = Rec."Match Confidence"::Accepted) or (Rec."Match Confidence" = Rec."Match Confidence"::Manual) then
            Rec."Match Confidence" := BankPmtApplRule.GetMatchConfidence(Rec.Quality);
    end;
