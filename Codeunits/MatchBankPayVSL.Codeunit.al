...
codeunit 71357861 "MatchBankPay_VSL"
{
    ...
    begin
        ...
        //Message(Format(TotalTimeRelatedPartyMatching));
    ...
    var
        ...
        CustLedgEntrTemp: Record "Cust. Ledger Entry" temporary;
    begin
        BankAccount.Get(BankAccReconciliationLine."Bank Account No.");
        SalesReceivablesSetup.Get();
        BankAccRec.SetRange("Bank Account No.", BankAccReconciliationLine."Bank Account No.");
        BankAccRec.SetRange("Statement No.", BankAccReconciliationLine."Statement No.");
        BankAccRec.SetRange("Statement Type", BankAccReconciliationLine."Statement Type");
        if BankAccRec.FindSet() then
            repeat
                ...
                CustLedgerEntry.SetFilter("Customer No.", BankAccRec."Account No._VSL");
                ...
                if CustLedgerEntry.FindSet() then
                    repeat
                        if not CustLedgEntrTemp.Get(CustLedgerEntry."Entry No.") then begin
                            CustLedgEntrTemp.init();
                            //CLEAR(NewRecord);
                            CustLedgEntrTemp.TRANSFERFIELDS(CustLedgerEntry);
                            CustLedgEntrTemp.Insert();
                            //CustLedgEntrTemp.Copy(CustLedgerEntry);
                            //CustLedgEntrTemp.Insert();
                        end;
        ...
        //Message(format(CustLedgEntrTemp.Count));
        if CustLedgEntrTemp.FindSet() then
            // Message(Format(CustLedgEntrTemp.Count));
            repeat
                TempLedgerEntryMatchingBuffer.Init();
                TempLedgerEntryMatchingBuffer.InsertFromCustomerLedgerEntry(
                  CustLedgEntrTemp, BankAccount.IsInLocalCurrency(), UsePaymentDiscounts);
            until CustLedgEntrTemp.Next() = 0;
        // Message(Format(CustLedgEntrTemp.Count));
    ...
    var
        ...
        BankAccRec: Record "Bank Acc. Reconciliation Line";
        VendLedgEntrTemp: Record "Vendor Ledger Entry" temporary;
    begin
        BankAccount.Get(BankAccReconciliationLine."Bank Account No.");
        ...
        if BankAccRec.FindSet() then
            repeat
                ...
                VendorLedgerEntry.SetFilter("Vendor No.", BankAccRec."Account No._VSL");
                ...
                if VendorLedgerEntry.FindSet() then
                    repeat
                        VendLedgEntrTemp.init();
                        VendLedgEntrTemp.TRANSFERFIELDS(VendorLedgerEntry);
                        VendorLedgerEntry.Insert();
                    //CustLedgEntrTemp.Insert();
                    until VendorLedgerEntry.Next() = 0;
            until BankAccRec.Next() = 0;
        if VendLedgEntrTemp.FindSet() then
            repeat
                TempLedgerEntryMatchingBuffer.Init();
                TempLedgerEntryMatchingBuffer.InsertFromVendorLedgerEntry(
                  VendLedgEntrTemp, BankAccount.IsInLocalCurrency(), UsePaymentDiscounts);
            until VendLedgEntrTemp.Next() = 0;
        // end;
    end;
    ...
    begin
        ...
        EmployeeLedgerEntry.SetFilter("Employee No.", BankAccReconciliationLine."Account No.");
    ...
    begin
        ...
        InitializeBankPmtApplSettings();
        BankPmtApplRule."Related Party Matched" := BankPmtApplRule."Related Party Matched"::Fully; //No
